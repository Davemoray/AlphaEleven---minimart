import * as React from "react";
import Svg, { Path } from "react-native-svg";
const SVGComponent = (props: any) => {
  const { color } = props;
  return (
    <Svg
      width={24}
      height={24}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <Path
        d="M8 16H15.263C19.751 16 20.433 13.18 21.261 9.07C21.5 7.883 21.619 7.29 21.332 6.895C21.045 6.5 20.495 6.5 19.394 6.5H6"
        stroke={color}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M8 16L5.379 3.515C5.27085 3.08232 5.02118 2.69821 4.66967 2.4237C4.31816 2.1492 3.88499 2.00007 3.439 2H2.5M8.88 16H8.469C7.105 16 6 17.151 6 18.571C5.99881 18.6262 6.00849 18.681 6.0285 18.7324C6.04851 18.7838 6.07846 18.8308 6.11662 18.8706C6.15479 18.9105 6.20043 18.9424 6.25094 18.9646C6.30145 18.9868 6.35584 18.9988 6.411 19H17.5"
        stroke={color}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M10.5 22C11.3284 22 12 21.3284 12 20.5C12 19.6716 11.3284 19 10.5 19C9.67157 19 9 19.6716 9 20.5C9 21.3284 9.67157 22 10.5 22Z"
        stroke={color}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M17.5 22C18.3284 22 19 21.3284 19 20.5C19 19.6716 18.3284 19 17.5 19C16.6716 19 16 19.6716 16 20.5C16 21.3284 16.6716 22 17.5 22Z"
        stroke={color}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};
export default SVGComponent;
